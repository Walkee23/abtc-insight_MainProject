...
<div class="relative group cursor-pointer">
    <div class="flex items-center gap-3">
        <div class="text-right">
            <p class="text-xs font-bold text-on-surface leading-tight">Test BHW</p>
    ...
    <div class="absolute right-0 top-full mt-2 w-48 bg-white rounded-xl shadow-lg border border-slate-100 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
        ...
        <div class="pt-28 px-10 pb-12">
            ...
            <div class="flex justify-between items-end mb-10">
                ...
                <a href="{{ route('bhw.referral') }}" class="px-6 py-3 bg-primary text-white rounded-xl font-bold flex items-center gap-2 shadow-xl shadow-primary/25 hover:bg-primary-container transition-all hover:-translate-y-0.5 active:translate-y-0">
            ...
            <div class="grid grid-cols-4 gap-6 mb-8">
                ...
                <div class="bg-white rounded-xl shadow-sm p-6 border border-slate-100">
                    <p class="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Total Submissions</p>
                    <div class="flex justify-between items-end">
                        <h3 class="text-4xl font-black text-blue-900">{{ $totalSubmissions }}</h3>
                    </div>
                </div>

                <!-- Pending Review -->
                <div class="bg-white rounded-xl shadow-sm p-6 border border-slate-100">
                    <p class="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Pending Review</p>
                    <h3 class="text-4xl font-black text-amber-700">{{ str_pad($pending, 2, '0', STR_PAD_LEFT) }}</h3>
                </div>

                <!-- Received at ABTC -->
                <div class="bg-white rounded-xl shadow-sm p-6 border border-slate-100">
                    <p class="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Received at ABTC</p>
                    <h3 class="text-4xl font-black text-slate-700">{{ $received }}</h3>
                </div>

                <!-- Completed Cycles -->
                <div class="bg-blue-700 rounded-xl shadow-sm p-6 text-white">
                    <p class="text-xs font-bold text-blue-200 uppercase tracking-wider mb-2">Completed Cycles</p>
                    <h3 class="text-4xl font-black">{{ $completed }}</h3>
                </div>
            </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-slate-100 bg-white">
                            ...
                            <tr class="hover:bg-slate-50 transition-colors group">
                                <!-- Reference No -->
                                <td class="px-6 py-4">
                                    <span class="text-xs font-bold text-blue-700 bg-blue-50 px-2 py-1 rounded">{{ $referral->reference_no }}</span>
                                </td>
                                <!-- Patient Name -->
                                <td class="px-6 py-4">
                                    <p class="text-sm font-bold text-slate-900">{{ $referral->patient_name }}</p>
                                </td>
                                <!-- Barangay -->
                                <td class="px-6 py-4 text-sm text-slate-600">
                                    {{ $referral->patient_barangay }}
                                </td>
                                <!-- Date Referred -->
                                <td class="px-6 py-4 text-sm text-slate-600">
                                    {{ \Carbon\Carbon::parse($referral->referral_date)->format('M d, Y') }}
                                </td>
                                <!-- Exposure Type -->
                                <td class="px-6 py-4">
                                    @if($referral->exposure_type === 'Bite')
                                    <span class="text-xs font-bold text-red-600 bg-red-50 px-2 py-1 rounded">Bite</span>
                                    @else
                                    <span class="text-xs font-bold text-slate-600 bg-slate-100 px-2 py-1 rounded">{{ $referral->exposure_type ?? 'N/A' }}</span>
                                    @endif
                                </td>
                                <!-- Status -->
                                <td class="px-6 py-4">
